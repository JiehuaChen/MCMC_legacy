...
dbinom_cont <- function(y,n, p){	
	dbinom_cont_values <- (gamma(n+1)/(gamma(y+1)*gamma(n-y+1)))*p^(y)*(1-p)^(n-y)
	return(dbinom_cont_values)
}
...
for(j in 1:n.chains){
	...
	for(i in 1:nIter){
		...
		lik.link.old <- log(dbinom_cont(Y, 1, logit.old)) + (dnorm(linkfunc.old, mean = linkfunc.hat, sd =sqrt(sigma2.draw), log = TRUE))
		linkfunc.new <- linkfunc.draw + rnorm(N, 0, sqrt(linkfunc.jump))		
		logit.new <- exp(linkfunc.new)/(1+exp(linkfunc.new))
		lik.link.new <- log(dbinom_cont(Y, 1, logit.new))+ dnorm(linkfunc.new, mean = linkfunc.hat, sd = sqrt(sigma2.draw), log = TRUE)
